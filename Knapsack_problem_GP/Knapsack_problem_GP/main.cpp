...
//  KnapsackProblem
//
//  Created by 熊之遥 曹袭亚 on 2017/11/12.
//  Copyright © 2017年 熊之遥 曹袭亚. All rights reserved.
//
#include <vector>
#include <iostream>
#include <ctime>
...
public:
    ...
    double Fitness_L(vector<CItem> items, vector<pair<int, int>> linkage) {
        double fit = 0;
        
        for (int i = 0; i < num; i++) {
            fit = fit + indicator[i] * items[linkage[i].first].Value();
        }
        fitness = fit;
        return fit;
    }
    ...
    void print_indicator() {
        ...
        for (int i = 0; i < num; i++) {
            cout << indicator[i] << " ";
            count = count + indicator[i];
        }
        cout << "num of ones " << count << endl;
        cout << endl;
    }
    ...
    bool Constrain_L(vector<CItem> items, double W, vector<pair<int, int>> linkage, bool print = false) {
        double c = 0;
        for (int i = 0; i < num; i++)
            c = c + indicator[i] * items[linkage[i].first].Weight();
        if (print)
            cout << "now weight: " << c << endl;
        if (c < W)
            return true;
        else {
            //cout << "now weight: " << c << endl;
    ...
    void resetIndicator(int n, bool s) {
        indicator[n] = s;
    }
    void recordAbs(vector<pair<int, int>> linkage) {
        for (int i = 0; i < num; i++) {
            indicator_abs[linkage[i].first] = indicator[i];
        }
    }
    bool getindicatorAbs(int n) {
        return indicator_abs[n];
    }
    bool getindicator(int n) {
        return indicator[n];
    }
};
...
CSolution * valid_init_L(int num, vector<CItem> items, double W, vector<pair<int, int>> linkage)
{
    bool valid_init = false;
    CSolution * true_init = new CSolution(num);
    CSolution * init = new CSolution(num);
    valid_init = init->Constrain_L(items, W, linkage);
    if (valid_init)
    {
        true_init = init;
        //delete init;
...
public:
	...
	vector<float> view_all_fitness() {
		vector<float> out_put_vec;
		for (int i = 0; i < population.size(); i++)
			out_put_vec.push_back(population[i]->getFit());
		return out_put_vec;
	}
    
     void reset(int index,  vector<pair<int, int>> linkage) {
         CSolution * temp = new CSolution(num);
         CSolution * old = population[index];
         if (index == pop_size() - 1) {
             cout << "old one" << endl;
             //population[index]->print_indicator();
         }
         for (int i = 0; i < temp->SolutionSize(); i++)
             temp->resetIndicator(i, old->getindicatorAbs(linkage[i].first));
    ...
    void Record(vector<pair<int, int>> linkage) {
        for (int i = 0; i < pop_size(); i++) {
            population[i]->recordAbs(linkage);
        }
    }
...
public:
    ...
    vector<CSolution *> Valid_CrossOver_L(CSolution * s1, CSolution * s2, vector<CItem> items, double W, vector<pair<int, int>> linkage) {
        int childern_number = 2;
        vector<CSolution *> new_s;
        int i = 0;
        while (i < childern_number)
        {
            vector<CSolution *> childs = CrossOver(s1, s2);
            if (childs[0]->Constrain_L(items, W, linkage))
            {
                i++;
                new_s.push_back(childs[0]);
            }
            if (childs[1]->Constrain_L(items, W, linkage))
            {
                i++;
                new_s.push_back(childs[1]);
            }
	...
	CPopulation Evolve_T(CPopulation POP, vector<CItem> items, double W){
		...
		for (int i = 0; i < selection_pool.pop_size(); i++)
		{
			...
			if (player1->getFit() > player2->getFit() && random_0to1() > 0.1)
    ...
    CPopulation Evolve_L(CPopulation POP, vector<CItem> items, double W, vector<pair<int, int>> linkage){
        int size = POP.pop_size();
        int n = (int)items.size();
       // int co_size = size * CrossOverPercent;
       // int mu_size = size * MutationRate;
        ...
        while (POP.pop_size() >0){
            ...
            vector<CSolution *> children = Valid_CrossOver_L(s1, s2, items, W, linkage);
            
            children[0]->Fitness_L(items, linkage);
            selection_pool.PutIn(children[0]);
            children[1]->Fitness_L(items, linkage);
            selection_pool.PutIn(children[1]);
        }
        selection_pool.shuffle_population();
        ...
        for (int i = 0; i < selection_pool.pop_size(); i ++)
        {
            ...
            CSolution * selected = selection_pool.Specific(i);
            CSolution * reborn = Mutation(selected);
            if (reborn->Constrain_L(items, W, linkage)){
                reborn->Fitness_L(items, linkage);
                mutated_pool.PutIn(reborn);
            }
            else{
                CSolution * reborn_ = valid_init_L(items.size(), items, W, linkage);
                reborn_->Fitness_L(items, linkage);
                mutated_pool.PutIn(reborn_);
            }
        }
...
public:
    ...
    CSolution * Random(string problem, int per_rec) {
        ...
        cout << (true_init->Fitness(items)) << endl;
		cout << "total_iter" << iteration << endl;
...
public:
    CRun(string prefix) {
        ...
        num = (int)items.size();
        linkage = InitialLinkage(num);
        cout << "solution size " << num << endl;
    }
    
    void ga_L(int popsize, int round, int learn_link, double mu_rate, double select_p, double co_rate, double eli_rate) {
        string resultfile = problem + "_ga_result_Linkage_no_round.txt";
        ofstream outfile(resultfile, ios::app);

		string resultfile_map = problem + "_ga_dotmap_Linkage_no_round.txt";
		ofstream outfile_map(resultfile_map, ios::app);
        ...
        for (int i = 0; i < round; i++) {
            ...
            generation = GA.Evolve_L(generation, items, W, linkage);
            //cout << "next round" << endl;
            generation.Sort();
            CSolution * temp = generation.PutOut();
            //temp->print_indicator();
           
            for (int k = 0; k < num; k++) {
                linkage[k].second = linkage[k].second + temp->getindicator(k);
            }
            if ( i%10 == 0)
			...
			// record_ all
			vector<float> all_fitness = generation.view_all_fitness();
			for (int i = 0; i < all_fitness.size(); i++)
				//cout << all_fitness[i] << " , ";
				outfile_map << all_fitness[i] << ",";
			outfile_map << endl;
			//

            bool print = true;
            if (i % learn_link == 0 && i!=0) {
                generation.Record(linkage);
                /*for (int k=0; k<linkage.size(); k++)
                {
                    // "first" and "second" are used to access
                    // 1st and 2nd element of pair respectively
                    cout << linkage[k].first << " "
                    << linkage[k].second << endl;
                }
                 */
                generation.Specific(popsize-1)->print_indicator();
                generation.Specific(popsize-1)->Constrain_L(items, W, linkage, print);
                sort(linkage.begin(), linkage.end(), [](const pair<int, int> &left, const pair<int, int> &right){
                    return left.second < right.second;
                });
               // try rearrange a little bit

				//rotate(linkage.begin(), linkage.begin() + randomPosition(num), linkage.end());
               
                for (int k = 0; k < generation.pop_size(); k++) {
                    generation.reset(k, linkage);
                    //linkage[k].second = 0;
                }
                cout << "learn linkage"<< endl;
                generation.Specific(popsize-1)->print_indicator();
                generation.Specific(popsize-1)->Constrain_L(items, W, linkage, print);
            }
    ...
    void random(int iteration, int per_rec) {
        CRandom rand(items, W, num, iteration/100);
    ...
    vector<pair<int, int>> InitialLinkage(int num) {
        vector<pair<int, int>> linkage;
        for (int i = 0; i < num; i++)
            linkage.push_back(make_pair(i, 0));
        return linkage;
    }
};
...
int main(int argc, const char * argv[]) {
	...
	/*
	generate_dataset generator(2000, 1e1, 1e7, 1e2, 1e3, true);
	...
	*/
    ...
    for (int i = 0; i < 2; i++) {
        ...
        start = clock();
        run.ga_L(popsize, round, 500, 2.0, 0.3, 1.2, 0.04);
        end = clock();
        cout << "Total Time: " <<  (double)(end - start) / CLOCKS_PER_SEC<< endl;
        
		
        start = clock();
		run.ga(popsize, round, 1.5, 0.3, 1.2, 0.04);
        end = clock();
        cout << "Total Time: " <<  (double)(end - start) / CLOCKS_PER_SEC<< endl;
